...
3. On any form, list or search view, click the magic-wand icon in the
   ...
   it (or **inside** a page), **place an existing field**, add a **page**
   or **group**, hide it, change its label, set a widget, restrict it to
   groups, or set modifiers (invisible / readonly / required). The
   ...
   **Related path** (for example `parent_id.email`) instead of a field
   type. Pages and buttons need a technical ``name`` in the view. If the
